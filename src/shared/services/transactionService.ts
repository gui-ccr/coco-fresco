...
type TxRow = { id: string; cat: string; value: number; when: string; note: string | null; payment_method: string | null };
...
export async function fetchTransactions(): Promise<Transaction[]> {
  const { data } = await api.get<TxRow[]>('/transactions', {
    params: { select: 'id,cat,value,when,note,payment_method', order: 'when.desc' },
  });
...
export async function updateTransaction(
  id: string,
  updates: Partial<Pick<Transaction, 'cat' | 'value' | 'when' | 'note' | 'payment_method'>>
): Promise<Transaction> {
