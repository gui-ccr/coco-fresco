...
const MENSAGENS = [
  "Coco Fresco 🥥",
  "Eu te amo, mãe 💚",
  "Que Deus abençoe as vendas 🙏",
  "Você é incrível, mãe ✨",
  "Bora faturar hoje! 💪",
  "O melhor coco da cidade 🌴",
  "Com fé, tudo dá certo 🌟",
...
export function DashboardView({ transactions, workDay }: Props) {
  ...
  // Filtra apenas transações de hoje
  ...
  const {
    ...
    const totalIncome = todayTxs
      .filter((t) => CATEGORY_META[t.cat].isIncome)
    ...
    const byArea = (area: string) =>
      todayTxs
        .filter(
          (t) =>
            !CATEGORY_META[t.cat].isIncome &&
            CATEGORY_META[t.cat].area === area,
        )
        .reduce((s, t) => s + t.value, 0);
  ...
  const pct = (v: number) =>
    totalExp > 0 ? Math.round((v / totalExp) * 100) : 0;
  return (
    <div>
      {/* ─── Header ─── */}
      <div
        ...
        style={{
          background:
            "linear-gradient(160deg, #064e3b 0%, #065f46 55%, #047857 100%)",
                ...
                {[
                  {
                    label: "Trabalho",
                    value: trabalhoExp,
                    color: AREA_COLORS.trabalho,
                    emoji: "🌴",
                  },
                  {
                    label: "Casa",
                    value: casaExp,
                    color: AREA_COLORS.casa,
                    emoji: "🏠",
                  },
                  {
                    label: "Gastos",
                    value: aleatorioExp,
                    color: AREA_COLORS.aleatorio,
                    emoji: "🛍️",
                  },
                ].map((item) => (
                  <div key={item.label}>
